  ...
  return (
    <Card className="mb-5">
      <CardHeader className="pb-2 pt-5">
        <CardTitle className="text-base flex items-center gap-2 font-semibold text-foreground">{icon}{title}</CardTitle>
      </CardHeader>
...
function Step({ number, title, children }: { number: number; title: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4">
      <div className="flex-shrink-0 w-7 h-7 rounded-full bg-primary/10 text-blue-600 dark:text-blue-300 flex items-center justify-center text-xs font-bold mt-0.5">{number}</div>
...
export default function OpenPlayGuidePage() {
  return (
    <div className="max-w-2xl">
      ...
      <Section title="For organisers — running a session" icon={<ListOrdered className="w-4 h-4 text-blue-400" />}>
        <div className="space-y-6">
          <Step number={1} title="Start a session">
            <p>On the <strong className="text-foreground">Open Play</strong> tab, name the session, set how many courts you have (1–15), pick Singles or Doubles, and choose whether it&apos;s <strong className="text-foreground">Rated</strong> (counts toward ELO) or casual.</p>
          </Step>
          <Separator />
          <Step number={2} title="Check players in">
            <p>Add <strong className="text-foreground">league members</strong> by name, or add <strong className="text-foreground">guests</strong> who aren&apos;t in the league. Everyone joins the queue in the order they check in.</p>
          </Step>
          <Separator />
          <Step number={3} title="Fill the courts">
            <p>Tap <strong className="text-foreground">Fill open courts</strong>. The Kitchen pulls the next players from the queue and splits each court into the two fairest teams by rating.</p>
          </Step>
          <Separator />
          <Step number={4} title="Record the winner">
            <p>When a game finishes, tap the <strong className="text-foreground">winning team</strong> on that court. Both teams return to the back of the queue, and the court is ready for the next group.</p>
          </Step>
          <Separator />
          <Step number={5} title="Handle real life">
            <p>Players can <strong className="text-foreground">rest</strong> (and rejoin later), and you can add latecomers any time — they go to the back of the queue.</p>
          </Step>
          <Separator />
          <Step number={6} title="End the session">
            <p>Tap <strong className="text-foreground">End</strong> when you&apos;re done. Wins, losses and games played are saved for everyone.</p>
          </Step>
        </div>
      </Section>
      ...
      <Section title="For players — joining in" icon={<Smartphone className="w-4 h-4 text-purple-500" />}>
        <div className="space-y-3 text-sm text-muted-foreground leading-relaxed">
          ...
          <p>Want to follow along from your seat? Ask for the <strong className="text-foreground">session link</strong> (the organiser&apos;s Share button). Open it on your phone to see who&apos;s on each court and your spot in the queue — it refreshes automatically. No crowding around the front desk.</p>
        </div>
      </Section>

      <Section title="How Open Play affects your rating" icon={<span className="text-base">📈</span>}>
        <div className="space-y-3 text-sm text-muted-foreground leading-relaxed">
          <p>A game updates league ELO only when <strong className="text-foreground">both conditions</strong> are met:</p>
          <ul className="list-disc pl-5 space-y-1">
            <li>The session is marked <strong className="text-foreground">Rated</strong>, and</li>
          ...
          <p>Open-play games count as a standard win for rating purposes (there&apos;s no point score to enter). Curious how the maths works?{' '}
            <Link href="/elo" className="text-blue-400 underline hover:text-green-300">See How ELO Works</Link>.
          </p>
        </div>
      </Section>
      ...
      <p className="text-xs text-center text-muted-foreground/80 pb-8">
        Back to <Link href="/dashboard" className="underline hover:text-muted-foreground">your leagues</Link>.
      </p>
    </div>
  )
}
