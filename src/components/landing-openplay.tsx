    ...
    <section className="bg-zinc-950 text-white py-20 sm:py-24 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4">
        <motion.div {...up(0)} className="text-center mb-12">
          <span className="inline-block text-[11px] font-bold uppercase tracking-[0.25em] text-blue-400 mb-3">The flagship feature</span>
          <h2 className="text-3xl sm:text-5xl font-extrabold italic uppercase tracking-tight leading-[0.95]">
            Open Play,<br /><span className="text-blue-400">done right.</span>
          </h2>
        ...
        <motion.div {...up(5)} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-10">
          {caps.map(c => (
            <div key={c.label} className="flex items-center gap-3 rounded-xl bg-zinc-900/60 border border-zinc-800 px-4 py-3">
              <c.icon className="w-5 h-5 text-blue-400 shrink-0" />
        ...
        <motion.div {...up(6)} className="text-center">
          <Link href="/play/new"
            className="inline-flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl px-7 py-3 text-base transition-colors">
            Run a session now →
          </Link>
