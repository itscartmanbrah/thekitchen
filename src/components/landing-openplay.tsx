...
export function LandingOpenPlay() {
  return (
    <section className="bg-slate-950 text-white py-20 sm:py-24 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4">
        <motion.div {...up(0)} className="text-center mb-12">
          ...
          <p className="text-slate-400 mt-5 max-w-2xl mx-auto text-base sm:text-lg">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-10">
          {styles.map((s, i) => (
            <motion.div key={s.name} {...up(i + 1)}
              className="rounded-2xl bg-slate-900 border border-slate-800 p-5 hover:border-green-500/50 transition-colors">
        ...
        <motion.div {...up(5)} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-10">
          {caps.map(c => (
            <div key={c.label} className="flex items-center gap-3 rounded-xl bg-slate-900/60 border border-slate-800 px-4 py-3">
