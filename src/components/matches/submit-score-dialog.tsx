    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-blue-500/10 border border-blue-100 rounded-lg px-3 py-2 text-xs text-blue-700 dark:text-blue-300">
              ...
              <div className="flex flex-col items-center gap-1 mb-1">
                {team1.map((p: any) => (
                  <div key={p.id} className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-foreground/90 truncate">{p.profiles.display_name}</span>
                  </div>
                ))}
              </div>
