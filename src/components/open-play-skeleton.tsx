...
export function OpenPlaySkeleton() {
  return (
    <div className="bg-slate-900 rounded-2xl p-4 sm:p-5 animate-pulse">
      <div className="grid grid-cols-4 gap-2 mb-5">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-slate-800 rounded-xl h-14" />
        ))}
      </div>
      <div className="h-3 w-16 bg-slate-800 rounded mb-3" />
      <div className="grid sm:grid-cols-2 gap-2.5 mb-6">
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i} className="bg-slate-800 rounded-xl h-28" />
        ))}
      </div>
      <div className="h-3 w-20 bg-slate-800 rounded mb-3" />
      <div className="space-y-1.5">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="bg-slate-800 rounded-lg h-10" />
        ))}
      </div>
    </div>
  )
