-- Allow multiple Open Play sessions per league, but two sessions can't use the
-- SAME court at overlapping times. Different courts may overlap freely.
-- (Booking conflict check from 038 is kept.)
...
begin
  ...
  -- 1) confirmed bookings on these courts must not overlap the window
  ...
  -- 2) another live Open Play session must not use the same court at the same time
  select string_agg(distinct co.name, ', ')
    into v_clash
  from play_sessions ps
  cross join lateral unnest(ps.court_ids) pc(court_id)
  join courts co on co.id = pc.court_id
  where ps.league_id = p_league_id
    and ps.ended_at is null
    and (ps.ends_at is null or ps.ends_at > now())
    and pc.court_id = any(p_court_ids)
    and tstzrange(ps.starts_at, coalesce(ps.ends_at, ps.starts_at + interval '4 hours')) && tstzrange(v_start, v_end);
  if v_clash is not null then
    raise exception 'Another Open Play session already uses % at an overlapping time. Pick a different time or court.', v_clash;
  end if;
